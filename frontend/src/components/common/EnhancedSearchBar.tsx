...
import { Search, X, Tag, Users, Hash, ChevronDown, ChevronUp } from 'lucide-react';
...
const EnhancedSearchBar: React.FC<EnhancedSearchBarProps> = ({ 
  ...
  // Determine if advanced options are active
  const hasActiveAdvancedOptions = activeLanguages.length > 0 || activeOrgs.length > 0 || activeTopics.length > 0;
    ...
    <div className="w-full max-w-4xl">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button
          ...
          className="flex items-center text-primary hover:text-primary-dark text-sm font-medium transition-colors focus:outline-none"
        >
          {showAdvancedSearch ? (
            <>
            ...
            <>
              ...
              Advanced Search {hasActiveAdvancedOptions && <span className="ml-1 px-1.5 py-0.5 bg-primary text-white text-xs rounded-full">{activeLanguages.length + activeOrgs.length + activeTopics.length}</span>}
            </>
          )}
        </button>
        ...
        {showAdvancedSearch && (
          <div ref={advancedSearchRef} className="space-y-4 p-4 border border-secondary-200 rounded-lg bg-secondary-50 transition-all duration-200 ease-in-out">
            <h3 className="font-medium text-primary text-sm mb-2">Add Filters</h3>
